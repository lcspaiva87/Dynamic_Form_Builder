      ...
      {forms.map((form) => (
        <div key={form.id}>
          <header className="mb-8">
            <h1 className="text-3xl font-bold mb-4">Respostas para: {form.title}</h1>
            <nav>
              <Link href={`/forms/${form.id}`} className="text-blue-500 hover:underline inline-block">
          ...
          <section>
            {responses.length > 0 && form.fields && form.fields.length > 0 ? (
                    ...
                    {responses.map((response) => (
                      <tr key={response.id} className="hover:bg-gray-50">
                        ...
                        {form.fields.map((field) => (
                          <td key={`${response.id}-${field.id}`} className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
              ...
              <p className="text-center text-gray-500 mt-4">Nenhuma resposta disponível.</p>
            )}
          </section>
        </div>
      ))}
